...
func (d *Dial) Rotate(code string) error {
	...
	// Count zeros during full rotations
	d.pin += off / d.mod
	...
	if d.value == 0 {
		...
		// Check if we passed zero marker
		if oldValue != 0 && (dir == 'L' && oldValue < d.value || dir == 'R' && oldValue > d.value) {
			d.pin += 1
		}
	}

	log.Printf("rotation: %2d %c %4s ⊜ %2d", oldValue, ri, code, d.value)
